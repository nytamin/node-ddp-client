...
class DDPClient extends events.EventEmitter {
  ...
  connect (cb) {
    ...
    if (cb) {
      ...
      let cbConnected
      cbConnected = () => {
        cb(undefined, this._isReconnecting);
        this.removeListener('failed', cbFailed)
      }
      cbFailed = (error) => {
        cb(error, this._isReconnecting);
        this.removeListener('connected', cbConnected)
      }
      this.once('connected', cbConnected)
      this.once('failed', cbFailed)
    }
