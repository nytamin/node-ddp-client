  ...
  _prepareHandlers () {
    ...
    this.socket.on("error", (error) => {
      ...
      if (
        ...
        typeof error === 'object' &&
        error.target &&
        error.target.statusCode === 301 && //redirect
        error.target.headers &&
        (
          error.target.headers.location || error.target.headers.Location
        )
      ) {
        this._redirected++
        var location = error.target.headers.location || error.target.headers.Location
        var m = location.match(/^(.*):\/\/([^/:]*):?(\d*)\/?(.*)/)
        if (m) {
          this.emit("info", "Redirecting to \"" + location + "\"")
          var protocol = m[1]
          var host = m[2]
          var port = m[3]
          var path = m[4]
          ...
          this.port = (
            parseInt(port) || 
            (protocol.match(/(http|ws)s/) && 443) ||
            (protocol.match(/(http|ws)/) && 80) ||
            this.port
          )
          this.ssl = this.port === 443;
          // this.path = path || this.path
          this.socket.close() // not this.close() because we want to keep the listeners
          this.connect()
          handledError = true
